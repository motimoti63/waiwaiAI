...
function onOpen() {
  ...
  ui.createMenu('カスタムメニュー')
    ...
    .addItem('AI比較を実行', 'processCheckedRows')
    .addItem('チェックボックス追加', 'addCheckboxes')
    .addToUi();
}
...
function addCheckboxes() {
  ...
  const range = sheet.getActiveRange(); // 現在選択されている範囲
  if (!range) {
  ...
  SpreadsheetApp.getUi().alert("チェックボックスを追加しました。");
}